...
class DbalSessionHandler implements \SessionHandlerInterface
{
     ...
     */
    public function __construct(Connection $connection, array $dbOptions = array())
    {
    ...
    public function write($id, $data)
    {
        ...
        try {
            ...
            if ('mysql' === $driver) {
                ...
                $stmt = $this->connection->prepare("MERGE INTO $dbTable USING DUAL ON($dbIdCol = :id) ".
                ...
                $stmt = $this->connection->prepare("UPDATE $dbTable SET $dbDataCol = :data, $dbTimeCol = :time WHERE $dbIdCol = :id");
                $stmt->bindParam(':id', $id, \PDO::PARAM_STR);
                $stmt->bindParam(':data', $encoded, \PDO::PARAM_STR);
                $stmt->bindValue(':time', time(), \PDO::PARAM_INT);
                $stmt->execute();
